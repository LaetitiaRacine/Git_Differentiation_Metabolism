...
dir.create(path = paste0(directory, "exp/scATACseq_bulkATACseq_R_SwitchGenome/", current_date))
directory_output = paste0(directory, "exp/scATACseq_bulkATACseq_R_SwitchGenome/", current_date, "/")
...
features_hg38_list = list.files(path = paste0(directory, "data/scATACseq/"), pattern = "_filtered_peak_bc_matrix_peaks.bed")
features_hg38_list = paste0(directory, "data/scATACseq/", features_hg38_list)
names(features_hg38_list) = str_extract(features_hg38_list, pattern = "(?<=scATACseq/)[:alnum:]+(?=_filtered)")
...
dir = pic_last_dir(paste0(directory, "exp/", "scATACseq_bulkATACseq_R_SwitchGenome_LiftOver/"))
features_hg19_list = list.files(path = paste0(dir, "/"), pattern = "_hg19_peaks.bed")
features_hg19_list = paste0(paste0(dir, "/"), features_hg19_list)
...
for (i in 1:length(features_hg19_list)) {
  ...
  list_peaks = features_hg19_list[[i]] %>%
    ...
    tidyr::separate(col = peak_hg19, into = c("seqnames", "start", "end"), remove = FALSE) %>%
...
for (i in 1:length(list_hg19_gr)) {
  ...
  # An overlap is calculated and used to assigned the peak as TRUE when overlapping with the annotation
...
# List directions
barcode_list = list.files(path = paste0(directory, "data/scATACseq/"), pattern = "_filtered_peak_bc_matrix_barcodes.tsv")
barcode_list = paste0(directory, "data/scATACseq/", barcode_list)
mtx_list = list.files(path = paste0(directory, "data/scATACseq/"), pattern = "_filtered_peak_bc_matrix_matrix.mtx")
...
for (i in 1:length(features_hg38_list)) { # really long !!!
  ...
  # Condition
  name_cond = str_extract(features_hg38_list[i], pattern = "(?<=scATACseq/)[:alnum:]+(?=_filtered)")
