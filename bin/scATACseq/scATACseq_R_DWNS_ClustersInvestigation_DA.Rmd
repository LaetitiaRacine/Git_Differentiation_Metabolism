...
This part of the code require a huge amount of memory and is extremely slow. We can only perform it on external cluster and it takes more than 24h. We did it on IFB (slurm job on long partition with memory 100GB accessible) and we saved it in the data folder.  "For sparse data (such as scATAC-seq), we find it is often necessary to lower the min.pct threshold in FindMarkers() from the default (0.1, which was designed for scRNA-seq data)."  
```{r, eval=FALSE}

# DefaultAssay(seurat_merged) <- 'peaks'
# Idents(seurat_merged) = "peaks_snn_res.0.25"
# 
# clust_markers = FindAllMarkers(object = seurat_merged,
#                       assay = "peaks",
...
#                       verbose = TRUE)
# 
# saveRDS(object = clust_markers, paste0(directory_output, "clust_markers_20230412.rds"))

```

```{r}

# Load the clust_markers table from IFBcluster
clust_markers = readRDS(paste0(directory, "data/scATACseq/clust_markers_20230412.rds"))
