...
expr = FetchData(object = seurat_obj, vars = "orig.ident")

for( i in 1:length(unique(seurat_obj$orig.ident))) {
  
    name_sample = unique(seurat_obj$orig.ident)[i]
    gr = seurat_obj[, which(expr == name_sample)]         
    gr = gr@assays$peaks@ranges

    # visualize on html file
    list_sample_chr[[name_sample]] = as.ggplot(expression(
      plotKaryotype(genome = "hg38", plot.type = 2) %>%
        kpPlotRegions(data=gr, avoid.overlapping = FALSE, col = color_code[name_sample]) %>%
        kpAddMainTitle(main = paste0("Peaks distribution on hg38 genome from ", name_sample), cex=1.5) %>%
...
do.call("grid.arrange", c(list_sample_chr, ncol = 2))
...
for (i in 1:length(chromosomes)) { 
  ...
  # visualize on html file
    ...
    for (j in 1:length(unique(seurat_obj$orig.ident))) {
      name_sample = unique(seurat_obj$orig.ident)[j]
      kp = plotKaryotype(plot.type = 1, chromosomes=chromosomes[i], plot.params = pp)
      gr = seurat_obj[, which(expr == name_sample)]         
      gr = gr@assays$peaks@ranges
      kpPlotRegions(kp, data=gr, border=color_code[name_sample], r0=j-1, r1=j-0.6)
      kpPlotDensity(kp, data=gr, data.panel = 1, r0=j-0.6, r1=j-0.2)
...
=> PROBLEM : all graphs are identical
