...
```{r, Working directories and external script, eval=FALSE}
...
start_time = Sys.time()
directory_output = "/shared/ifbstor1/home/lracine/scATACseq/scATACseq_R_DWNS_GeneActivity_GeneCoverage"
seurat_obj = readRDS("/shared/ifbstor1/home/lracine/scATACseq/merged_seurat_qc_filter_annot_normreduc.rds")
annot_hg38 = readRDS("/shared/ifbstor1/home/lracine/scATACseq/hg38_annotations_gr.rds")
source(file = "/shared/ifbstor1/home/lracine/functions_constants.R")
...
=> to try on external cluster
```{r}

path_bam_CTRL = paste0(directory, "/data/scATAcseq/CTRL_possorted_bam.bam")
path_bam_DON = paste0(directory, "/data/scATAcseq/DON_possorted_bam.bam")
path_bam_2DG = paste0(directory, "/data/scATAcseq/CTRL_possorted_bam.bam")
path_bam_AOA = paste0(directory, "/data/scATAcseq/CTRL_possorted_bam.bam")
...
for(i in 1:nrow(chrom_gr)){
  ...
  chr = chrom_gr$chr[i]
  max = (chrom_gr %>% dplyr::filter(seqnames == chr))$end

  # find max value of peak to put the same axe on all coverages
  kp <- plotKaryotype(genome = "hg38", chromosomes = chr)
  temp <- kpPlotBAMCoverage(kp, data=path_bam_CTRL, max.valid.region.size = max)
  max = temp$latest.plot$computed.values$max.coverage
  temp <- kpPlotBAMCoverage(kp, data=path_bam_DON, max.valid.region.size = max)
  if (max < temp$latest.plot$computed.values$max.coverage) {max = temp$latest.plot$computed.values$max.coverage}
  temp <- kpPlotBAMCoverage(kp, data=path_bam_2DG, max.valid.region.size = max)
  if (max < temp$latest.plot$computed.values$max.coverage) {max = temp$latest.plot$computed.values$max.coverage}
  temp <- kpPlotBAMCoverage(kp, data=path_bam_AOA, max.valid.region.size = max)
  if (max < temp$latest.plot$computed.values$max.coverage) {max = temp$latest.plot$computed.values$max.coverage}
  rm(kp)
    
  # draw coverage
  
  png(filename = paste0(directory_output, "coverage_kpbam_", chrom, ".png"),
       width = 1000, height = 1400)
  
  kp <- plotKaryotype(genome = "hg38", chromosomes = chr)
  kpAddBaseNumbers(kp, tick.dist = 50000, add.units = TRUE)
  kp <- kpPlotBAMCoverage(kp, data=path_bam_CTRL, max.valid.region.size = max, col = color_code["CTRL"], r1=0.4)
  kpAddLabels(kp, "CTRL", r0=0, r1=0.5, label.margin = 0.05)
  kp <- kpPlotBAMCoverage(kp, data=path_bam_DON, max.valid.region.size = max, col = color_code["DON"], r1=0.6)
  kpAddLabels(kp, "DON", r0=0.5, r1=1, label.margin = 0.05)
  kp <- kpPlotBAMCoverage(kp, data=path_bam_2DG, max.valid.region.size = max, col = color_code["2DG"], r1=0.8)
  kpAddLabels(kp, "2DG", r0=1, r1=1.5, label.margin = 0.05)
  kp <- kpPlotBAMCoverage(kp, data=path_bam_AOA, max.valid.region.size = max, col = color_code["AOA"], r1=1)
  kpAddLabels(kp, "AOA", r0=1.5, r1=2, label.margin = 0.05)
...
**Possibility 3 : calcul average number of fragments per cell per condition **  
Require a lot of memory. Not possible on basic computer, run it on external cluster.
```{r, eval=FALSE}
dir = pic_last_dir(paste0(directory, "exp/", "scATACseq_R_NormalizationReduction/"))
mat = readRDS(paste0(dir, "/", "mat_fragments.rds"))
```

```{r,load mat on IFBcluster}
mat = readRDS("/shared/ifbstor1/home/lracine/scATACseq/mat.rds")
```

```{r}

# we sample 100 cells of each condition
list_graph = list()
...
for (i in 1:length(unique(seurat_obj$orig.ident))) {
  ...
  cells_to_keep = sample(str_subset(colnames(mat), cond), 100)
...
rm(mat, mat_cond, mat_stat)
saveRDS(list_graph, paste0(directory_output, "list_graph.rds"))
saveRDS(list_graph_stat, paste0(directory_output, "list_graph_stat.rds"))

```

```{r, fig.height=14, fig.width=20}

to_draw = unlist(list_graph, recursive=FALSE)
do.call("grid.arrange", c(to_draw, ncol = 3))

to_draw_stat = unlist(list_graph_stat, recursive=FALSE)
do.call("grid.arrange", c(to_draw_stat, ncol = 3))

```

```{r, fig.show="hide"}

ggsave(plot = do.call("grid.arrange", c(to_draw, ncol = 3)),
       filename = paste0(directory_output, "chromosome_distribution_nbfrag.svg"))
ggsave(plot = do.call("grid.arrange", c(to_draw_stat, ncol = 3)),
       filename = paste0(directory_output, "chromosome_distribution_stat.svg"))
