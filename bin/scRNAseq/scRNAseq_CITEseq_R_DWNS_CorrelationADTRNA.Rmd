...
This code aims at :    
- seing the correlation between RNA and protein for CD34 and CD133    
...
We work from the count matrix from seurat object. The **"count"** slot contains raw data whereas the **"data"** slot contains normalized data with CLR algorithm. 
```{r}
...
We depicted the data on plots to visualize the proteins' distribution in the population. We can identified outliers.
...
For UMAP representation of protein expression (ADT), we set a max cutoff at 2.6 for CD34 and 2.2 for CD133 to have a more readable scale. This means that all values above those cutoffs have the same color on the scale. For RNA expression, we use the predefined max and min cutoff (95% and 5%) because we don't have precise value to provide.
```{r, UMAP ADT RNA}
...
- choose a groups resolution (two groups or three groups)  
