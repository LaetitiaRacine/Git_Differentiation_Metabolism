...
- filtering out genes detected in less than 3 cells  
...
<br>
Based on those plots, we can place a threshold. By doing so, we decide to disregard all the cells with less than *1000 genes and those with more than 6500 genes*. The median number for genes detected per cell is between 2731 and 3880 genes depending on the condition (cf. results websummary CellRanger), this is a positive indication to the threshold we established. We can be quite drastic with this threshold since we are not looking for a rare population. 
...
Cells presenting a high percentage of mitochondrial RNA and a low percentage of ribosomal RNA are either dead or dying. When we trace the scatter plot percent.ribo vs percent.mt, we can see two distinct populations: a first one on the left with a low percentage of mitochondrial RNA and a percentage of ribosomal RNA between 10 and 50% indicating living cells, and a second population on the right characterized by a high percentage of mitochondrial RNA and a low percentage of ribosomal RNA. In order to remove most of the dead cells and still maintain a sufficient amount of data we apply a filter with the following parameters: keep the cells with **less than 25% of mitochondrial RNA** and **more than 5% of ribosomal RNA**. 
...
As we are not searching for rare population, we decide to remove genes expressed in less than 3 cells. 
...
We check the characteristic of each filtered object. As we can see now, the objects don't present the same number of genes (initially they were all at 36601) and the number of cells is slower than the initial ones (cf. 2023-03-13_scRNAseq_CITEseq_R_CreateSeuratObject). We will dig deeper in the objects to appreciate the number of cells/genes removed by each filter. 
...
for(i in 1:number_conditions) {
  ...
  temp = data.frame(condition = rep(names(filtered_list_seurat_obj[i]), 5),
                    ...
                    nbgenes_detected_per_cell = c(
                      ...
                      sd(filtered_list_seurat_obj[[i]]$nFeature_RNA)),
                    percent_MT_RNA = c(
                      ...
                      sd(filtered_list_seurat_obj[[i]]$percent.mt)),
                    percent_RB_RNA = c(
                      ...
                      sd(filtered_list_seurat_obj[[i]]$percent.ribo)))
...
ctrl_sub = merge(x = filtered_list_seurat_obj[["CTRL"]],
                 y = filtered_list_seurat_obj[["CTRL2"]])
tab_ctrl = data.frame(condition = rep("CTRL1&2", 5),
                      statistic = c("min", "median", "mean", "max", "sd"),
                      nbgenes_detected_per_cell = c(
                        min(ctrl_sub$nFeature_RNA), 
                        median(ctrl_sub$nFeature_RNA), 
                        mean(ctrl_sub$nFeature_RNA), 
                        max(ctrl_sub$nFeature_RNA),
                        sd(ctrl_sub$nFeature_RNA)))
tab_ctrl  %>%
  kable() %>%
  kable_styling()
...
meanUMI_per_gene_corrected : the cells with zero UMI for this gene are discarded for this calculation so we have the mean UMI of the cells that expressed the gene studied.    
...
list_ind_genes = list()
list_histo_max = list()
list_histo_max_zoom = list()
list_histo_sum = list()
list_histo_nbcells = list()
...
for (i in 1:length(list_mat_detected)) {
    ...
    ind_genes = data.frame(
      ...
      sdUMI_per_gene = rowSds(temp_mat))
    ind_genes[is.na(ind_genes)] = 0
       
    list_ind_genes[[i]] = ind_genes
    
    # Visualize distribution
    histo_max = ggplot(data.frame(maxUMI_per_gene = rowMaxs(temp_mat)), aes(x=maxUMI_per_gene)) +
      geom_histogram(binwidth = 20, fill = "#7AB5FF", col = "#1351A1") +
      theme(axis.text.x = element_text(size = 8, angle = 90)) +
      ggtitle(names(list_mat_detected[i]))
    histo_max_zoom = ggplot(data.frame(maxUMI_per_gene = rowMaxs(temp_mat)), aes(x=maxUMI_per_gene)) +
      geom_histogram(binwidth = 1, fill = "#7AB5FF", col = "#1351A1") +
      coord_cartesian(xlim = c(0,50)) +
      theme(axis.text.x = element_text(size = 8, angle = 90)) +
      ggtitle(names(list_mat_detected[i]))
    histo_sum = ggplot(data.frame(sumUMI_per_gene = rowMaxs(temp_mat)), aes(x=sumUMI_per_gene)) +
      geom_histogram(binwidth = 20, fill = "#7AB5FF", col = "#1351A1") +
      ggtitle(names(list_mat_detected[i]))
    histo_nbcells = ggplot(data.frame(nbcells_expressing_gene = rowSums(temp_binary)), aes(x=nbcells_expressing_gene)) +
      geom_histogram(binwidth = 20, fill = "#7AB5FF", col = "#1351A1") +
      ggtitle(names(list_mat_detected[i]))
    
    list_histo_max[[i]] = histo_max
    list_histo_max_zoom[[i]] = histo_max_zoom
    list_histo_sum[[i]] = histo_sum
    list_histo_nbcells[[i]] = histo_nbcells
...
do.call("grid.arrange", c(list_histo_max, ncol=3))
do.call("grid.arrange", c(list_histo_max_zoom, ncol=3))
do.call("grid.arrange", c(list_histo_sum, ncol=3))
do.call("grid.arrange", c(list_histo_nbcells, ncol=3))
...
for (i in 1:length(list_mat_detected)) {
    ...
    list_genes_allcells[[i]] = rownames(temp_binary %>% 
                                          dplyr::mutate(keep = ifelse(
                                            rowSums(temp_binary) == ncol(temp_mat), 
                                            TRUE, FALSE)) %>%
...
names(list_genes_allcells) = names(list_mat_detected)
...
for (i in 1:length(list_genes_allcells)) {
  cat(length(list_genes_allcells[[i]]), "genes common to all cells in", 
      names(list_genes_allcells[i]), ": \n",
      list_genes_allcells[[i]], "\n")
}

print("Common genes for CTRL1 and CTRL2")
Reduce(intersect, list(list_genes_allcells[["CTRL"]],
                       list_genes_allcells[["CTRL2"]]))

print("Common genes for CTRL1, CTRL2, CTRLaK, DON, DONaK, 2DG, 2DGaK, AOA, AOAaK")
Reduce(intersect, list(list_genes_allcells[["CTRL"]],
                       list_genes_allcells[["CTRL2"]],
                       list_genes_allcells[["DON"]],
                       list_genes_allcells[["DONaK"]],
                       list_genes_allcells[["2DG"]],
                       list_genes_allcells[["2DGaK"]],
                       list_genes_allcells[["AOA"]],
                       list_genes_allcells[["AOAaK"]],
                       list_genes_allcells[["CTRLaK"]]))

print("Common genes for CTRL1, CTRL2, CTRLaK, DON, DONaK, 2DG, 2DGaK, AOA, AOAaK, VPA")
Reduce(intersect, list(list_genes_allcells[["CTRL"]],
                       list_genes_allcells[["CTRL2"]],
                       list_genes_allcells[["DON"]],
                       list_genes_allcells[["DONaK"]],
                       ...
                       list_genes_allcells[["AOA"]],
                       list_genes_allcells[["AOAaK"]],
                       list_genes_allcells[["VPA"]],
                       list_genes_allcells[["CTRLaK"]]))

print("Common genes for all cells of DON but not in others conditions")
setdiff(list_genes_allcells[["DON"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["DONaK"]],
          list_genes_allcells[["2DG"]],
          list_genes_allcells[["2DGaK"]],
...
print("Common genes for all cells of 2DG but not in others conditions")
setdiff(list_genes_allcells[["2DG"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["DONaK"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of AOA but not in others conditions")
setdiff(list_genes_allcells[["AOA"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["DONaK"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of VPA but not in others conditions")
setdiff(list_genes_allcells[["VPA"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["DONaK"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of DONaK but not in others conditions")
setdiff(list_genes_allcells[["DONaK"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["VPA"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of CTRLaK but not in others conditions")
setdiff(list_genes_allcells[["CTRLaK"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["VPA"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of 2DGaK but not in others conditions")
setdiff(list_genes_allcells[["2DGaK"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["VPA"]],
          list_genes_allcells[["DON"]],
...
print("Common genes for all cells of AOAaK but not in others conditions")
setdiff(list_genes_allcells[["AOAaK"]], 
        c(list_genes_allcells[["CTRL"]],
          list_genes_allcells[["CTRL2"]],
          list_genes_allcells[["VPA"]],
          list_genes_allcells[["DON"]],
...
saveRDS(list_genes_allcells, file = paste0(directory_output, "list_genes_allcells.rds"))
