...
We work here from the folder we received from Cochin platform, with all output files from CellRanger software. For each multiplexing sample, we have a folder with all needed files for *count matrix* creation, which are stored in : [sample name]/outs/per_sample_outs/sample[1ou2]/count. For each simple sample obtained with updated CellRanger software, files are stored in folders named as : [sample name]/outs/filtered_feature_bc_matrix. To simplify, the files we use in this code were moved and renamed to follow the same nomenclature.  
To easily distinguish the gene (assay = RNA) from the protein (assay = ADT) for the CD34 marker, protein detection is indicated under CD34.1. For CD133 marker, there is no problem because the associated gene has a different name known as PROM1.  
...
data.frame(list_seurat_obj[["CTRL"]]@assays$RNA@data[1:10, 1:5]) %>% 
  ...
  kable_styling()
```
When creating seurat objects, the raw counts are stored in both the counts and data slot at the beginning. Then, the NormalizeData() function will work off of the counts slots and overwrite the data slot when the counts slot will still contain the raw data.  
